"""
Pinturillo P2P - Juego de dibujo multijugador
...
class Paint3:
    """
    Clase principal del juego Pinturillo con arquitectura P2P.
    ...
    def start_game(self):
        ...
        # Enviar información del juego a todos
        game_data = {
            "type": "start_game",
        ...
        self.process_message(game_data, None)  # Procesar también en el host
    ...
    def send_message(self, event=None):
        ...
        if message:
            # Verificar respuesta correcta ANTES de enviar (si no soy el dibujante)
            is_correct = False
            if self.game_active and not self.am_i_drawing:
                if message.lower().strip() == self.current_word.lower():
                    is_correct = True
                    self.scores[self.my_name] = self.scores.get(self.my_name, 0) + 10
                    self.update_players_list()
                    
                    # Mostrar mensaje local de éxito
                    self.add_chat_message("SISTEMA", "¡Adivinaste correctamente! 🎉")
                    
                    # Enviar notificación de adivinanza correcta (sin mostrar la palabra)
                    correct_data = {
                        "type": "correct_guess",
                        "name": self.my_name,
                        "scores": self.scores
            ...
            # Solo enviar el mensaje al chat si NO es la respuesta correcta
            if not is_correct:
                chat_data = {
                    "type": "chat",
                    "name": self.my_name,
